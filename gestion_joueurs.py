...
def creer_dossier(pseudo_du_joueur):
    ...
    chemin_dossier = os.path.join(path.Path("joueurs"), pseudo_du_joueur) # Le chemin du nouveau dossier est composé du nom du dossier parent et du pseudo du joueur
...
def supprimer_dossier(pseudo_du_joueur):
    ...
    if pseudo_du_joueur in joueurs_existants():
        chemin_dossier = os.path.join(path.Path(f"joueurs/{pseudo_du_joueur}"))
        if os.listdir("joueurs") != []:
            for file in os.listdir(chemin_dossier):
                os.remove(os.path.join(path.Path(f"{chemin_dossier}/{file}")))
